    ...
    def validate_supplier_list(self):
        for d in self.suppliers:
            ...
            if warn_rfqs:
                standing = frappe.db.get_value("Supplier Scorecard", d.supplier, "status")
                frappe.msgprint(
                    _("{0} currently has a {1} Supplier Scorecard standing, and RFQs to this supplier should be issued with caution.").format(d.supplier, standing),
                    title=_("Caution"),
                    indicator="orange",
                )
...
def get_rfq_containing_supplier(doctype, txt, searchfield, start, page_len, filters):
    ...
    rfq_data = frappe.db.sql(
		...
		limit %(page_len)s offset %(start)s """.format(filters.get("supplier"), filters.get("company"), conditions),
        {
            "page_len": page_len,
            "start": start
        },
