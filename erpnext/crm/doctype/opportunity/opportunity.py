...
class Opportunity(TransactionBase):
    ...
    def validate(self):
        self._prev = frappe._dict(
            {
                "contact_date": frappe.db.get_value("Opportunity", self.name, "contact_date") if (not cint(self.get("__islocal"))) else None,
                "contact_by": frappe.db.get_value("Opportunity", self.name, "contact_by") if (not cint(self.get("__islocal"))) else None,
            }
        )
    ...
    def make_new_lead_if_required(self):
        ...
        if (not self.get("party_name")) and self.contact_email:
            ...
            if not lead_name:
                ...
                lead = frappe.get_doc({"doctype": "Lead", "email_id": self.contact_email, "lead_name": sender_name or "Unknown"})
    ...
    @frappe.whitelist()
    def declare_enquiry_lost(self, lost_reasons_list, date_to_reopen, detailed_reason=None):
        if not self.has_active_quotation():
            ...
            if date_to_reopen:
                frappe.db.set(self, 'date_to_reopen', date_to_reopen)
    ...
    def has_ordered_quotation(self):
        if not self.with_items:
            return frappe.get_all("Quotation", {"opportunity": self.name, "status": "Ordered", "docstatus": 1}, "name")
        else:
            return frappe.db.sql(
                """
				select q.name
...
def make_quotation(source_name, target_doc=None):
    def set_missing_values(source, target):
        ...
        if company_currency == quotation.currency:
            ...
            exchange_rate = get_exchange_rate(quotation.currency, company_currency, quotation.transaction_date, args="for_selling")
